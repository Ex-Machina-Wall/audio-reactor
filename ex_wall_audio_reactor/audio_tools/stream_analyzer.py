...
from ex_wall_audio_reactor.audio_tools.fft import get_fft
...
class StreamAnalyzer:
    ...
    def __init__(
        ...
        if self.smoothing_length_ms > 0:
            ...
            self.feature_buffer = NumpyDataBuffer(len(self.smoothing_kernel), len(self.fft), dtype=np.float32,
                                                  data_dimensions=2)
    ...
    def update_features(self, n_bins=3):
        ...
        self.fft = get_fft(latest_data_window, self.rate, self.FFT_window_size, log_scale=self.log_features)
